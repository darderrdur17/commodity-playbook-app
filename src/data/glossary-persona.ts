...
export interface PersonaGlossaryGuide {
  headline: string;
  tip: string;
  /** Category names to highlight first — full glossary remains available */
  priorityCategories: string[];
}

/** Desk vocabulary priorities by persona — aligned with resume archetypes */
export const PERSONA_GLOSSARY_GUIDES: Record<ApiPersona, PersonaGlossaryGuide> = {
  FRESH_GRAD: {
  ...
  ANALYST_TRADER: {
    ...
    tip: "Prioritise pricing, risk, and market intelligence terms — the gap is explaining what your analysis enabled on the desk, not the maths itself.",
    priorityCategories: [
      "Pricing & Derivatives",
      "Risk & P&L",
      "Market Intelligence & Analytics",
...
export function getPersonaGlossaryGuide(persona: string | null | undefined) {
  if (!persona) return null;
  return PERSONA_GLOSSARY_GUIDES[persona as ApiPersona] ?? null;
}
